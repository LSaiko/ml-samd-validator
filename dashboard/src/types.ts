...
export interface MetricDrift { metric: string; delta: number; confidence: number; band: Band }
export interface DriftReport { overall_band: Band; requires_human_review: boolean; metrics: MetricDrift[] }
export interface SubgroupFinding { subgroup: string; metric: string; delta: number; flagged: boolean; band: Band }
